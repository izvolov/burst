...
namespace burst
{
    ...
    /*!
        ...
        разнородные объекты. Причём, это могут быть как члены одной иерархии классов, так и
        совершенно разные типы. Например, можно поместить одновременно bool, double, std::string,
        std::vector<int> и т.п.
            Нужен в тех случаях, когда на этапе компиляции известно, какой тип объекта нужно
        достать, но неизвестно, под каким индексом он расположен.

            Ключевая особенность в том, что объекты плотно расположены в памяти. То есть они лежат
        в едином буфере подряд, друг за другом. Это снимает лишний уровень косвенности при
        обращении к ним по сравнению с тем, если бы это был, например, массив указателей на
        базовый класс, как это обычно делается для хранения в массиве классов из одной иерархии.
     */
    class dynamic_tuple
    {
    public:
