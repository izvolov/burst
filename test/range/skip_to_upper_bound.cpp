...
BOOST_AUTO_TEST_SUITE(skip_to_upper_bound)
    BOOST_AUTO_TEST_CASE(skipping_by_value_in_empty_range_results_empty_range)
    {
    ...
    BOOST_AUTO_TEST_CASE(skipping_by_existing_value_makes_range_begin_after_last_occurence_of_the_value)
    {
        ...
        (
            ...
            boost::begin(values) + 3, boost::end(values)
        );
    }

    BOOST_AUTO_TEST_CASE(skipping_by_value_greater_than_all_values_in_range_makes_range_empty)
    {
        auto values = {1, 2, 3};
        auto sorted_range = boost::make_iterator_range(values);
    ...
    BOOST_AUTO_TEST_CASE(skipping_by_value_less_than_all_elements_in_range_does_not_modify_range)
    {
        auto values = {3, 2, 1};
        auto sorted_range = boost::make_iterator_range(values);
    ...
    BOOST_AUTO_TEST_CASE(skipping_by_value_absent_in_sorted_range_makes_range_begin_at_first_value_greater_than_desired)
    {
